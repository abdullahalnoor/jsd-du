          ...
          <div class="tile">
            ...
            <div class="tile-body">
                ...
                <div class="mb-3">
                  ...
                  <input class="form-control" value="{{old('name',$role->name)}}" name="name" type="text" placeholder="Enter Name">
                  @error('name')
                ...
                <div class="mb-3">
                  ...
                  <input class="form-control" value="{{old('slug',$role->slug)}}" name="slug" type="text" placeholder="Enter  Slug">
                  @error('slug')
                  <b class="form-text text-danger">{{$message}} </b>
                  @enderror
                </div>
            ...
            <label class="form-check-label   m-2" style="margin-left:5px">
               <input class="form-check-input"  name="permission_id[]" 
               value="{{$permissionItem->id}}"  type="checkbox" 
                 @forelse ($role->permissions as $rolePer)
                  {{$rolePer->id == $permissionItem->id ? 'checked' : '' }}
              @empty
